	...
	<c:forEach var="member" items="${members}" varStatus="loop">
		<tr>
			...
			<td>	<%-- 이름 열 : 첫번째줄 배경색을 yellow, 마지막 줄을 cyan --%>
				<c:if test="${loop.first}">  <%-- 첫번째줄 --%>
					<span style="background-color: yellow;">${member.name }</span>
				</c:if>
				<c:if test="${loop.last}">  <%-- 마지막줄 --%>
					<span style="background-color: yellow;">${member.name }</span>
				</c:if>
				<c:if test="${not (loop.first or loop.last)}">  <%-- 첫 마지막 줄 빼고 --%>
					${member.name }
				</c:if>
			</td>
			<td>
